...
var parsePomfile = function(context) {
  var pomfile = context.pomfile;  // pomfile should be a buffer
  ...
  if (pomfile.length > 0) {
    parsedPomfile.requestline = pomfile[0]; // first line
    instructions = pomfile.slice(1);
  }

  instructions.forEach(function(instruction){
    if (instruction.indexOf('?') === 0) {
      parsedPomfile.querystringparams.push(instruction);
    } else if (instruction.indexOf('@') === 0) {
      parsedPomfile.httpom_directives.push(instruction);
    } else {
      parsedPomfile.headers.push(instruction); // for now, assume it's a header if it doesn't start with @ or ?. TODO: parse this properly to determine if it's a valid header line or not!
    }
  });

  log(parsedPomfile);
  context.parsedPomfile = parsedPomfile;
  return context;
};
